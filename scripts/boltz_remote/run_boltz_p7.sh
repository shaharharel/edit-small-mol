#!/usr/bin/env bash
# P7 boltz runner — variant of run_boltz_parallel.sh with configurable RUN_DIR.
# Resumable: skips YAMLs whose output directory already exists with confidence JSON.
set -uo pipefail

N_PARALLEL=${N_PARALLEL:-2}
RUN_DIR="${RUN_DIR:-$HOME/boltz_run_p7}"
YAML_DIR="${YAML_DIR:-$RUN_DIR/yamls}"
OUT_DIR="${OUT_DIR:-$RUN_DIR/results}"
...
for env in boltz boltz2 quris; do
    if conda env list 2>/dev/null | grep -q "^${env} "; then
        conda activate "$env"; echo "[runner] activated: $env"; break
    fi
done
if ! command -v boltz >/dev/null; then
    echo "[runner] ERROR: boltz not found"; exit 1
fi
