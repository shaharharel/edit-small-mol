...
| HF10 | **C-ARM-PATCH on every fresh DiffSBDD clone** | DiffSBDD's conditional model has `xh_pocket = xh0_pocket.detach().clone()` at `conditional_model.py:154,177`. That detach kills gradient flow back to anything modifying `pocket['one_hot']` — including our C-arm adapter. **Without the patch the adapter is frozen at random init for the entire fine-tune.** T4 v2 had the patch (so v2 results are valid). ai-gpu2 v2.5 was missing it until 2026-05-14 22:30 — caught during smoke test (adapter grad = 0.0). Run `anchordiff/covind/patch_diffsbdd.sh` after every fresh DiffSBDD clone; it's idempotent. | any machine running C+D | patch script committed | `anchordiff/covind/patch_diffsbdd.sh` |
