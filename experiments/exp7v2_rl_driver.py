"""EXP7-v2 LO eval — Per-pair RL driver for the 54-pair v2 benchmark (runs ON a100-b).

Strategy:
  - A:                       covFT prior + DAP RL with composite reward (per-pair training)
  - baseline_prior_anchor:   covFT prior, NO RL, sample with anchor as seed

REST server per target must be running (use exp6_rest_server.py).

Cohort CSVs land at: data/exp7_v2_benchmark/_rl/<pair_id>_<strategy>/sampled.csv

Resumable: skips if sampled.csv already exists with > 1000 bytes.
"""
...
# Port allocation per v2 target (server already running)
PORTS = {
...
def render_sample_toml(target_key, pair_id, strategy, agent_file, seed_file, out_dir, num_smiles=NUM_SMILES):
...
def process_pair_cell(pair: dict, strategy: str, force: bool = False) -> dict:
    ...
    if strategy == "A":
        ...
        return {"status": "ok", "strategy": strategy, "dt_rl": dt_rl, "dt_sample": dt_s, "cohort_path": cohort_path}
    elif strategy == "baseline_prior_anchor":
        ...
        return {"status": "fail", "reason": f"unknown strategy: {strategy}"}
...
def main():
    ...
    for cell in queue:
        ...
        if elapsed > args.budget_sec:
            print(f"[exp7v2-rl] BUDGET HIT after {elapsed/60:.1f} min. Stopping.", flush=True)
            break
        t0 = time.time()
        ...
        elapsed = time.time() - t_start
        print(f"[exp7v2-rl] {cell['pair_id']}_{cell['strategy']} -> {out.get('status','?')} "
