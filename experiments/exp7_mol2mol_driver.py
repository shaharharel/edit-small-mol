"""EXP7 LO eval — mol2mol prior driver (runs ON a100-b).

Two conditions per pair:
  - mol2mol_baseline: vanilla mol2mol prior, NO RL, sample 5500 mols with anchor as seed
  - mol2mol_RL:       vanilla mol2mol prior + composite RL reward (50 steps), then sample 5500

Mirrors exp7_rl_driver.py but uses vanilla mol2mol prior:
    PRIOR_FILE = /home/shaharh_quris_ai/edit-small-mol/models/reinvent4_mol2mol_prior.prior

REST server per target must be running (use exp6_rest_server.py).

Cohort CSVs land at: data/exp7_lo_benchmark/_rl/<pair_id>_<condition>/sampled.csv
Where condition in {mol2mol_baseline, mol2mol_RL}.

Resumable: skips if cohort CSV already exists with > 1000 rows.
"""
...
def render_sample_toml(target_key, pair_id, condition, agent_file, seed_file, out_dir, num_smiles=5500):
...
def process_pair_cell(pair: dict, condition: str, force: bool = False, prior_file: str = PRIOR_FILE_DEFAULT) -> dict:
    """Run one (pair, condition) cell. Returns metrics dict."""
    ...
    if condition == "mol2mol_RL":
        ...
        return {"status": "ok", "condition": condition, "dt_rl": dt_rl, "dt_sample": dt_s,
...
def main():
    ...
    ap.add_argument("--pairs_csv", required=True, help="CSV: pair_id, target_key, anchor_smiles, drug_smiles, condition")
    ap.add_argument("--out_results", required=True)
    ap.add_argument("--prior_file", default=PRIOR_FILE_DEFAULT)
    ap.add_argument("--budget_sec", type=int, default=18000, help="Total wallclock seconds (default 5h)")
    args = ap.parse_args()
    ...
    for cell in queue:
        ...
        if elapsed > args.budget_sec:
            print(f"[exp7-mol2mol] BUDGET HIT after {elapsed/60:.1f} min. Stopping.", flush=True)
            break
        t0 = time.time()
        ...
        elapsed = time.time() - t_start
        print(f"[exp7-mol2mol] {cell['pair_id']}_{cell['condition']} -> {out.get('status','?')} "
