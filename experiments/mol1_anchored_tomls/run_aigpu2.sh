#!/bin/bash
# ai-gpu2 shard: 4 cohorts
set -eo pipefail
LOG="$HOME/mol1_anchored_sampling_aigpu2.log"
date -u +"[%FT%TZ] ai-gpu2 shard start" | tee -a "$LOG"
...
date -u +"[%FT%TZ] ai-gpu2 shard ALL DONE" | tee -a "$LOG"
