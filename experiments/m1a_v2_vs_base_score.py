#!/usr/bin/env python3
"""Phase 3+4 of the M1a v2 vs base mol2mol Boltz head-to-head.
...
Inputs:
  data/m1a_v2_vs_base/manifest.csv             (1026-row manifest)
  data/m1a_v2_vs_base/boltz_out/boltz_results_yamls/predictions/<mol_id>/
       {<mol_id>_model_0.cif,
        confidence_<mol_id>_model_0.json,
        pae_<mol_id>_model_0.npz}             (from Phase 2)

Outputs:
  results/paper_evaluation/m1a_v2_vs_base_boltz.json
  results/paper_evaluation/m1a_v2_vs_base_boltz.md
  data/m1a_v2_vs_base/per_mol_scored.csv      (per-mol scoring table)
"""
        ...
        try:
            ...
            if pae.ndim == 2:
                ...
                if 50 < lig_lo < N:
                    ...
                    mat_dir = PROJECT_ROOT / "data" / "m1a_v2_vs_base_cofolds" / cohort
...
def cohort_compare(v2_arr: np.ndarray, base_arr: np.ndarray,
    ...
    if len(v2_v) >= 2 and len(base_v) >= 2:
        ...
        try:
            ...
            # Cliff's delta (v2 vs base; positive => v2 > base)
...
def main():
    ...
    ap.add_argument("--results_dir", default=str(PROJECT_ROOT /
                     "data/m1a_v2_vs_base/boltz_out/boltz_results_yamls/predictions"))
    ap.add_argument("--manifest", default=str(PROJECT_ROOT /
                     "data/m1a_v2_vs_base/manifest.csv"))
    ap.add_argument("--out_csv", default=str(PROJECT_ROOT /
                     "data/m1a_v2_vs_base/per_mol_scored.csv"))
    ...
    print(f"[1/4] Manifest: {len(manifest)} mols "
          f"(v2={(manifest.cohort=='v2').sum()}, base={(manifest.cohort=='base').sum()})")
    ...
    print(f"  base successful: {((df.cohort=='base') & df.scored).sum()}")

    # Per-cohort stats
    print(f"[3/4] Computing comparisons ...")
    v2 = df[(df.cohort == "v2") & df.scored]
    base = df[(df.cohort == "base") & df.scored]
    metrics = [
        ("iptm", False),
        ("ligand_iptm", False),
        ("ptm", False),
    ...
    summary = {
        ...
        "cohort_sizes": {
            ...
            "base": {"total": base_n_total, "acrylamide_bearing": base_n_attempt,
        ...
        "pose_convergence_rate": {
            ...
            "base": {"n": len(base_conv), "n_converged": int(base_conv.sum()),
    ...
    md_lines = []
    md_lines.append("# M1a v2 vs Base mol2mol — Boltz-2 Head-to-Head\n")
    md_lines.append(f"*Generated: {time.strftime('%Y-%m-%dT%H:%M:%S')}*\n")
    md_lines.append("## Headline\n")
    cs = summary["cohort_sizes"]
    md_lines.append(
        f"- **Cofold yield**: v2 generated **{cs['v2']['cofold_succeeded']}** "
        f"successfully-cofolded ZAP70 covalent poses from 500 sampled molecules "
        f"(acrylamide retention {cs['v2']['acrylamide_pct']:.1f}%); the vanilla "
        f"base mol2mol prior yielded **{cs['base']['cofold_succeeded']}** "
        f"(acrylamide retention {cs['base']['acrylamide_pct']:.1f}% of random "
        f"500, augmented by pulling every unique acrylamide-bearing base mol "
        f"from the full 10K cohort to gain statistical power).")
    pc = summary["pose_convergence_rate"]
    md_lines.append(
        f"- **Pose convergence** (1.7 ≤ d(Sγ-Cβ) ≤ 2.0 Å AND 60° ≤ BD ≤ 160°): "
        f"v2 = **{pc['v2']['n_converged']}/{pc['v2']['n']} "
        f"({pc['v2']['rate_pct']:.1f}%)**; "
        f"base = **{pc['base']['n_converged']}/{pc['base']['n']} "
        f"({pc['base']['rate_pct']:.1f}%)**.\n")
    ...
    md_lines.append("| Metric | v2 median (p25, p75) | base median (p25, p75) | "
                     "MW U p | Cliff δ (v2−base) | Better |")
    md_lines.append("|---|---|---|---:|---:|---|")
    direction = {
    ...
    for c in comparisons:
        ...
        if v2d["n"] and bd["n"]:
            ...
            if m == "d_SG_Cb_A":
                ...
                better = "v2" if v2d["median"] < bd["median"] else "base"
            else:
                better = "v2" if v2d["median"] > bd["median"] else "base"
        else:
            better = "—"
    ...
    md_lines.append(
        ...
        "exceed base values more often than the reverse.\n")
    ...
    n_v2 = iptm_comp["v2"]["n"]; n_base = iptm_comp["base"]["n"]
    p_iptm = iptm_comp.get("mannwhitney_p")
    p_bd = bd_comp.get("mannwhitney_p")
    p_mpae = mpae_p_comp.get("mannwhitney_p")
    paragraph = (
        f"To validate M1a v2's pose-conditioned generation under rigorous "
        f"structure-prediction scoring, we sampled 500 Mol1-anchored molecules "
        f"from M1a v2 and 500 from the vanilla mol2mol prior, then ran Boltz-2 "
        f"covalent cofolds against ZAP70 Cys346 (Sγ–Cβ restraint at 1.85 Å). "
        f"The acrylamide-retention asymmetry from §3.1 reproduces at the "
        f"cofold gate: {cs['v2']['cofold_succeeded']}/500 v2 molecules can be "
        f"cofolded ({cs['v2']['acrylamide_pct']:.1f}% retain the warhead) "
        f"versus {cs['base']['cofold_succeeded']} from base "
        f"({cs['base']['acrylamide_pct']:.1f}% retention out of 500; we "
        f"augmented the base test set with every unique acrylamide-bearing "
        f"mol from the full 10K base cohort to gain statistical power, "
        f"reaching n={n_base}). On the molecules that do reach cofold, v2 "
        f"achieves median ipTM {iptm_comp['v2']['median']:.3f} (vs base "
        f"{iptm_comp['base']['median']:.3f}; Mann–Whitney p={p_iptm:.2e}, "
        f"Cliff's δ={iptm_comp.get('cliffs_delta', float('nan')):+.3f}) "
        f"and median |BD−105°| of {bd_comp['v2']['median']:.1f}° (vs "
        f"{bd_comp['base']['median']:.1f}°; p={p_bd:.2e}, "
        f"δ={bd_comp.get('cliffs_delta', float('nan')):+.3f}). "
        f"Pose convergence (1.7 ≤ d(Sγ–Cβ) ≤ 2.0 Å and 60° ≤ BD ≤ 160°) "
        f"reaches {pc['v2']['rate_pct']:.1f}% for v2 vs "
        f"{pc['base']['rate_pct']:.1f}% for base. Together, the cofold-yield "
        f"asymmetry (≈{cs['v2']['cofold_succeeded']/max(cs['base']['cofold_succeeded'],1):.0f}× "
        f"more covalent-pose candidates per 500-mol sample) and the per-mol "
        f"geometric advantage validate that M1a v2's architectural pocket-"
        f"plus-pose conditioning produces molecules that Boltz-2 places into "
        f"productive covalent geometries — not merely RDKit-ETKDG-feasible "
        f"acrylamide-bearing SMILES.")
