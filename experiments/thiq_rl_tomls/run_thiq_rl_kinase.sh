#!/bin/bash
# thiq_rl_kinase runner — FiLM REST server + RL
set -eo pipefail
LOG="$HOME/thiq_rl_kinase.log"
date -u +"[%FT%TZ] thiq_rl_kinase START" | tee -a "$LOG"
...
work=/home/shaharh_quris_ai/edit-small-mol/results/paper_evaluation/mol1_rl/thiq_rl_kinase
rm -rf "$work"; mkdir -p "$work"; cd "$work"
cp /home/shaharh_quris_ai/edit-small-mol/experiments/thiq_rl_tomls/thiq_rl_kinase.toml .
date -u +"[%FT%TZ] RL launching" | tee -a "$LOG"
timeout 5400 reinvent ./thiq_rl_kinase.toml -d cuda 2>&1 | tee -a "$LOG" || {
