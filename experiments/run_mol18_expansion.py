#!/usr/bin/env python3
"""
Mol 18 Expansion Pipeline — End-to-End Molecule Generation & Scoring.

Seed compound (Mol 18, free base):
    C=CC(=O)N1CCN(c2ncnc(NC3(CC)CCNCC3)n2)CC1
...
Usage:
    conda run --no-capture-output -n quris python -u experiments/run_mol18_expansion.py
"""
...
def phase1_mmp_enumeration(mol18_smi, zap70_pairs):
    """Apply known MMP transforms from ZAP70 training data to Mol 18."""
    with timer("Phase 1a: MMP enumeration from ZAP70 transforms"):
        mol18 = Chem.MolFromSmiles(mol18_smi)
        if mol18 is None:
            print("    ERROR: Mol 18 is invalid!")
            return set()
        ...
        for reactant_smarts, product_smarts in beneficial_edits:
            try:
                ...
                if pattern is None:
                    continue
                matches = mol18.GetSubstructMatches(pattern)
                if not matches:
                    continue
                ...
                products = AllChem.ReplaceSubstructs(mol18, pattern, replacement)
        ...
        # For each pair where mol_a or mol_b is structurally similar to Mol 18,
        # extract the structural difference and try to apply it
        mol18_fp = compute_fingerprints([mol18_smi])[0]
        ...
        if len(all_pair_smiles) > 0:
            ...
            # Find molecules similar to Mol 18
            sims = tanimoto_kernel_matrix(mol18_fp.reshape(1, -1), pair_fps)[0]
            sim_mask = sims > 0.3  # At least 30% similar
            similar_mols = [all_pair_smiles[i] for i in range(len(all_pair_smiles)) if sim_mask[i]]
            print(f"    Found {len(similar_mols)} molecules with Tanimoto > 0.3 to Mol 18")
            ...
            for _, row in zap70_pairs.iterrows():
                ...
                # If mol_a is similar to mol18, apply the transform mol_a -> mol_b to mol18
        ...
        mol18_can = canonicalize(mol18_smi)
        generated.discard(mol18_can)
...
def phase1_crem_generation(mol18_smi):
    """Generate analogs of Mol 18 using CReM context-aware fragment replacement."""
...
def phase1_brics_recombination(mol18_smi, mol_data):
    """BRICS decomposition of Mol 18 + recombination with fragments from potent ZAP70 mols."""
    with timer("Phase 1c: BRICS recombination"):
        ...
        mol18_frags = set()
        try:
            mol18_frags = BRICS.BRICSDecompose(mol18)
            print(f"    Mol 18 BRICS fragments: {len(mol18_frags)}")
            for f in mol18_frags:
                print(f"      {f}")
        except Exception as e:
            print(f"    Mol 18 decomposition error: {e}")
            return set()
        ...
        # Recombine — only use Mol 18 fragments + top 30 most common other fragments
        # to keep combinatorial explosion manageable
        generated = set()
        MAX_BRICS = 5000

        # Prioritize fragments: all Mol 18 frags + top fragments from potent molecules
        other_frags = all_fragments - mol18_frags
        # Limit other fragments to keep BRICSBuild tractable
        other_frags_list = list(other_frags)[:40]
        selected_frags = list(mol18_frags) + other_frags_list
        print(f"    Selected {len(selected_frags)} fragments for recombination "
              f"({len(mol18_frags)} from Mol 18 + {len(other_frags_list)} from training)")
...
def phase2_reinvent_configs(mol18_smi):
    ...
    with timer("Phase 2: REINVENT4 config generation"):
        ...
        mol2mol_config = f"""# REINVENT4: Mol2Mol Optimization for Mol 18 Expansion
# Seed: {mol18_smi}
...
smiles_file = "{RESULTS_DIR}/mol18_seed.smi"
...
[[stage]]
chkpt_file = "mol18_mol2mol_stage1.chkpt"
termination = "simple"
max_score = 0.7
min_steps = 10
max_steps = 30
...
"""
        with open(configs_dir / "mol18_mol2mol.toml", "w") as f:
            f.write(mol2mol_config)

        # Write seed SMILES file
        with open(RESULTS_DIR / "mol18_seed.smi", "w") as f:
            f.write(f"{mol18_smi}\n")

        print(f"    Mol2Mol config written to {configs_dir / 'mol18_mol2mol.toml'}")
        print(f"    Seed SMILES written to {RESULTS_DIR / 'mol18_seed.smi'}")
        ...
        return {
            ...
            "mol2mol_config": str(configs_dir / "mol18_mol2mol.toml"),
            "seed_file": str(RESULTS_DIR / "mol18_seed.smi"),
        }
...
def score_all_candidates(smiles_list, mol_data, mol18_smi, n_seeds=N_SEEDS):
    ...
    sim_to_mol18 = tanimoto_kernel_matrix(cand_fps, mol18_fp.reshape(1, -1))[:, 0]
    ...
    results_df["tanimoto_to_mol18"] = sim_to_mol18
...
def generate_report(results_df, generation_stats, mol18_smi, mol_data):
    """Generate comprehensive HTML report."""
    with timer("Generating HTML report"):
        RESULTS_DIR.mkdir(parents=True, exist_ok=True)

        # Mol 18 properties
        mol18_props = compute_mol_properties(mol18_smi)
        mol18_svg = mol_to_svg(mol18_smi, size=(350, 280))
        ...
        if "source" in results_df.columns:
            for method in results_df["source"].unique():
                ...
                method_stats[method] = {
                    ...
                    "mean_sim_mol18": float(subset["tanimoto_to_mol18"].mean()),
...
<head>
<meta charset="utf-8">
<title>Mol 18 Expansion Pipeline Report</title>
<style>
...
.mol18-card { display: flex; align-items: flex-start; gap: 30px; }
.mol18-svg { flex-shrink: 0; }
.mol18-info { flex-grow: 1; }
.mol18-info table { border-collapse: collapse; width: 100%; }
.mol18-info td { padding: 4px 12px; border-bottom: 1px solid #eee; }
.mol18-info td:first-child { font-weight: bold; color: #555; width: 140px; }
...
""")
        ...
        # Title
        html.append(f"""
<h1>Mol 18 Expansion Pipeline Report</h1>
<p>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
<p>Target: ZAP70 (CHEMBL2803) | Seed: Mol 18 (pyrimidine-piperazine acrylamide)</p>
""")
...
""")
        ...
        # Mol 18 card
        html.append(f"""
<h2>2. Mol 18 (Seed Compound)</h2>
<div class="card mol18-card">
<div class="mol18-svg">{mol18_svg}</div>
<div class="mol18-info">
<table>
<tr><td>SMILES</td><td class="smi">{mol18_smi}</td></tr>
<tr><td>MW</td><td>{mol18_props.get('MW', 'N/A'):.1f}</td></tr>
<tr><td>LogP</td><td>{mol18_props.get('LogP', 'N/A'):.2f}</td></tr>
<tr><td>HBA / HBD</td><td>{mol18_props.get('HBA', 'N/A')} / {mol18_props.get('HBD', 'N/A')}</td></tr>
<tr><td>TPSA</td><td>{mol18_props.get('TPSA', 'N/A'):.1f}</td></tr>
<tr><td>Rot. Bonds</td><td>{mol18_props.get('RotBonds', 'N/A')}</td></tr>
<tr><td>QED</td><td>{mol18_props.get('QED', 'N/A'):.3f}</td></tr>
<tr><td>SA Score</td><td>{mol18_props.get('SA_Score', 'N/A')}</td></tr>
<tr><td>Heavy Atoms</td><td>{mol18_props.get('HeavyAtoms', 'N/A')}</td></tr>
<tr><td>Rings</td><td>{mol18_props.get('Rings', 'N/A')}</td></tr>
</table>
</div>
</div>
""")
...
""")
        ...
        for i, row in top50.iterrows():
            ...
            sim_m18 = row.get("tanimoto_to_mol18", 0)
...
<td>{stats['mean_QED']:.3f}</td>
        ...
        for i, row in top10.iterrows():
            ...
            pic50 = row.get("film_pIC50", 0)
            std = row.get("film_std", 0)
            sim = row.get("tanimoto_to_mol18", 0)
            source = row.get("source", "unknown")
            ...
            if sim >= 0.7:
                rationale_parts.append(f"Very similar to Mol 18 (Tc={sim:.2f})")
            elif sim >= 0.4:
                rationale_parts.append(f"Moderate similarity to Mol 18 (Tc={sim:.2f})")
            else:
                rationale_parts.append(f"Novel scaffold vs Mol 18 (Tc={sim:.2f})")
...
""")
        ...
        html.append(f"""
<div class="footer">
<p>Mol 18 Expansion Pipeline | Edit-Small-Mol Project | {datetime.now().strftime('%Y-%m-%d')}</p>
...
def main():
    print("=" * 70)
    print("MOL 18 EXPANSION PIPELINE")
    print(f"Seed: {MOL18_SMILES}")
    ...
    if phase1_cache.exists():
        ...
        # 1a. MMP Enumeration
        mmp_mols = phase1_mmp_enumeration(MOL18_SMILES, zap70_pairs)
        for smi in mmp_mols:
            source_map[smi] = "MMP"
        ...
        # 1b. CReM Generation
        crem_mols = phase1_crem_generation(MOL18_SMILES)
        ...
        brics_mols = phase1_brics_recombination(MOL18_SMILES, mol_data)
        ...
        # Remove training molecules and Mol 18 itself
        train_set = set(canonicalize(s) for s in mol_data["smiles"])
        mol18_can = canonicalize(MOL18_SMILES)
        all_generated -= train_set
        all_generated.discard(mol18_can)
    ...
    reinvent_info = phase2_reinvent_configs(MOL18_SMILES)
    ...
    scored_df = score_all_candidates(smiles_list, mol_data, MOL18_SMILES, n_seeds=N_SEEDS)
    ...
    if len(scored_df) > 0:
        report_path = generate_report(scored_df, results["generation"], MOL18_SMILES, mol_data)
