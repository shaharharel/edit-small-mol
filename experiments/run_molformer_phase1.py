#!/usr/bin/env python3
"""
Run MoLFormer-XL through Phase 1 evaluation (same conditions as other embedders).

Steps:
  1. Cache MoLFormer embeddings for all 88K molecules
  2. Run EditDiff architecture on within-assay split, 3 seeds
  3. Save results to all_results.json (updates Phase 1 section)

Usage:
    python -u experiments/run_molformer_phase1.py
"""
...
def cache_molformer_embeddings(smiles_list):
    """Compute and cache MoLFormer embeddings for all molecules."""
    cache_file = CACHE_DIR / "molformer-xl.npz"
    ...
    if cache_file.exists():
        ...
        # Check if all molecules are covered
    ...
    print("Computing MoLFormer-XL embeddings...")
    from src.embedding.molformer import MoLFormerEmbedder
    embedder = MoLFormerEmbedder(device='cpu', batch_size=64)
...
def main():
    print("=" * 60)
    print("MoLFormer-XL Phase 1 Evaluation")
    print("=" * 60)
    ...
    t0 = time.time()
    emb_dict, emb_dim = cache_molformer_embeddings(all_smiles)
    ...
    if "phase1" not in all_results:
        all_results["phase1"] = {}
    all_results["phase1"]["molformer-xl"] = molformer_result

    with open(results_file, "w") as f:
        json.dump(all_results, f, indent=2, default=str)

    # Print summary
    agg = molformer_result["aggregated"]
    print(f"\n{'=' * 60}")
    print("MoLFormer-XL Phase 1 Results:")
