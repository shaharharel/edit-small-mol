"""PPO-v2: SMARTS-ladder dense reward + Murcko diversity filter.

DIFFS vs ppo_v1 (ppo_mol2mol.py):
  * RESTScorer now sends predictor_id="ladder" (routes to SMARTS-ladder composite)
  * Murcko-scaffold diversity bucket (REINVENT default), bucket=10, minscore=0.4
  * Per-step diagnostic adds 'warhead_rate' (fraction of samples with ANY ladder match)
  * --reward_predictor_id CLI arg (default "ladder")
"""
...
logger = logging.getLogger("ppo_v2_ladder")


# =============================================================================
# Scorer with selectable predictor_id
# =============================================================================
class RESTScorer:
    def __init__(self, url: str, predictor_id: str = "ladder",
                 predictor_version: str = "v2", inp_fmt: str = "smiles",
                 timeout: float = 600.0):
        import requests
        self._requests = requests
        self.url = url
        self.params = dict(predictor_id=predictor_id,
                           predictor_version=predictor_version,
                           inp_fmt=inp_fmt)
        self.timeout = timeout

    def __call__(self, smiles: List[str]) -> np.ndarray:
        body = [{"input_string": s if s else "", "query_id": str(i)}
...
def _validate_smiles(out_smiles_list):
    from rdkit import Chem
    return [(bool(s) and Chem.MolFromSmiles(s) is not None) for s in out_smiles_list]


def _warhead_match_rate(smiles_list):
    """Fraction of valid mols matching ANY ladder tier (≥ Michael acceptor)."""
    from rdkit import Chem
    pats = [Chem.MolFromSmarts(p) for p in [
        "C=CC(=O)N1Cc2ccccc2C1",
        "C(=O)C=C[#7]",
        "C=CC(=O)N",
        "[CX3]=[CX3][CX3]=[OX1]",
    ]]
    ...
    for s in smiles_list:
        ...
        if m is None:
            continue
        n_tot += 1
        if any(m.HasSubstructMatch(p) for p in pats):
            n_match += 1
...
def sample_batch(agent_old, input_pool, batch_size, device, randomize=True):
    ...
    loader = tud.DataLoader(dataset, batch_size=batch_size, shuffle=False,
...
class PPOTrainer:
    def __init__(self, agent, agent_old, prior, scorer, input_pool, device, cfg,
                 diversity_bucket: int = 10, diversity_minscore: float = 0.4):
        ...
        # REINVENT-style Murcko diversity filter (persistent across batches)
        self.scaffold_counts: Counter = Counter()
        self.div_bucket = int(diversity_bucket)
        self.div_minscore = float(diversity_minscore)

    def _apply_diversity(self, smiles_list: List[str], rewards: np.ndarray) -> np.ndarray:
        """Zero out rewards for over-represented Murcko scaffolds.
        Only applies to rewards >= minscore (cheap rewards pass through)."""
        from rdkit import Chem
        from rdkit.Chem.Scaffolds import MurckoScaffold
        out = rewards.copy()
        n_pen = 0
        for i, (s, r) in enumerate(zip(smiles_list, rewards)):
            if r < self.div_minscore:
                continue
            if not s:
                continue
            try:
            ...
            if n >= self.div_bucket:
                ...
                n_pen += 1
        if n_pen:
            logger.info(f"  diversity filter zeroed {n_pen} rewards (bucket={self.div_bucket})")
        return out
    ...
    def step(self, step_idx):
        ...
        if n_valid > 0:
            ...
            for i, ok in enumerate(valid_mask_list):
                if ok:
                    rewards[i] = valid_scores[j]; j += 1

        # diversity filter: zero rewards from over-represented Murcko scaffolds
        ...
        last_loss = float("nan")
        clip_frac_acc = kl_acc = ent_acc = 0.0
        n_epochs_run = 0
        for k in range(self.cfg.ppo_epochs):
            ...
            if finite_mask.sum().item() == 0:
                logger.warning(f"[step {step_idx} epoch {k}] no finite log-probs"); continue
        ...
        with open(csv_log_path, "w", newline="") as fh:
            ...
            for step_idx in range(self.cfg.outer_steps):
                ...
                logger.info(
                    ...
                    f"war_any={log['warhead_any_rate']:.3f} thiq_exact={log['thiq_exact_rate']:.3f} "
                    f"kl={log['mean_kl_to_prior']:+.3f} ent={log['mean_entropy']:.3f} "
                    f"clipf={log['clip_fraction']:.3f} loss={log['loss']:+.3f} "
                    f"valid={log['n_valid']}/{self.cfg.batch_size} dt={dt:.1f}s"
                )
...
def build_scorer(reward_url, predictor_id="ladder"):
    return RESTScorer(reward_url, predictor_id=predictor_id)
...
def main():
    ...
    p.add_argument("--reward_predictor_id", default="ladder",
                   help="Server predictor_id: 'ladder' (v2 dense), 'composite' (DAP-binary), 'film'")
    ...
    p.add_argument("--checkpoint_out", type=str, required=True)
    ...
    scorer = build_scorer(args.reward_url, predictor_id=args.reward_predictor_id)
    cfg = PPOConfig(clip_eps=args.clip_eps, beta_kl=args.beta_kl,
                    alpha_ent=args.alpha_ent, ppo_epochs=args.ppo_epochs,
                    lr=args.lr, batch_size=args.batch_size, outer_steps=args.steps)
    trainer = PPOTrainer(agent, agent_old, prior, scorer, smilies, device, cfg,
                         diversity_bucket=args.div_bucket,
                         diversity_minscore=args.div_minscore)
    output_path = Path(args.output).resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    logger.info(f"PPO cfg: {cfg}; reward_predictor_id={args.reward_predictor_id}")
