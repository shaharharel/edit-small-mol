"""PPO-v3: GRPO (Group Relative Policy Optimization, Shao 2024).

Key change vs v2: advantage = (R - group_mean) / (group_std + eps), where the
"group" is a set of G completions sampled from the SAME prompt. We sample
multiple completions per prompt (G=8 by default; batch_size becomes the number
of prompts × G), then within-prompt normalize. The value head is dropped.

Other diffs vs v2:
  * advantage normalization within prompt-group
  * β_KL = 0.04 (Shao paper)
  * Still uses SMARTS-ladder reward + Murcko diversity filter (carried over)
  * No baseline deque (replaced by group-relative)
"""
...
logger = logging.getLogger("ppo_v3_grpo")


class RESTScorer:
    def __init__(self, url, predictor_id="ladder", predictor_version="v3",
                 inp_fmt="smiles", timeout=600.0):
        import requests
        self._requests = requests; self.url = url
        self.params = dict(predictor_id=predictor_id,
                           predictor_version=predictor_version, inp_fmt=inp_fmt)
...
def sample_grpo_batch(agent_old, input_pool, n_prompts, group_size, device, randomize=True):
    """Sample G completions per prompt for n_prompts prompts.
    Returns inputs (n_prompts*G), outputs (n_prompts*G), old_nlls, prompt_ids."""
    # Choose n_prompts unique seeds, then replicate each G times.
    seed_smilies = list(np.random.choice(input_pool, size=n_prompts, replace=True))
    # Build replicated input list (G copies of each, in blocks)
    rep_inputs = []
    prompt_ids = []
...
def compute_group_advantages(rewards: np.ndarray, prompt_ids: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    """For each group (same prompt_id), normalize: A = (R - mean) / (std + eps)."""
...
class GRPOConfig:
    ...
    ppo_epochs: int = 4
    lr: float = 1e-5
    n_prompts: int = 8       # prompts per outer step
...
class GRPOTrainer:
    ...
    def step(self, step_idx):
        ...
        last_loss = float("nan")
        clip_frac_acc = kl_acc = ent_acc = 0.0; n_epochs_run = 0
        ...
        with open(csv_log_path, "w", newline="") as fh:
            ...
            for step_idx in range(self.cfg.outer_steps):
                ...
                logger.info(
                    ...
                    f"ent={log['mean_entropy']:.3f} cf={log['clip_fraction']:.3f} "
...
def main():
    ...
    logger.info(f"Loaded {len(smilies)} seed SMILES")
    scorer = RESTScorer(args.reward_url, predictor_id=args.reward_predictor_id)
    cfg = GRPOConfig(clip_eps=args.clip_eps, beta_kl=args.beta_kl,
    ...
    trainer = GRPOTrainer(agent, agent_old, prior, scorer, smilies, device, cfg,
                          div_bucket=args.div_bucket, div_minscore=args.div_minscore)
    output_path = Path(args.output).resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    logger.info(f"GRPO cfg: {cfg}")
