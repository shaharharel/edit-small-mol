#!/usr/bin/env python3
"""
Run Uni-Mol v1 and/or v2 through Phase 1 evaluation.

Usage:
    python -u experiments/run_unimol_phase1.py --version v1
    python -u experiments/run_unimol_phase1.py --version v2
"""
...
def cache_unimol_embeddings(smiles_list, version='v1', model_size='84m'):
    """Compute and cache Uni-Mol embeddings for all molecules."""
    if version == 'v1':
        cache_name = "unimol-v1"
        model_name = 'unimolv1'
    else:
        cache_name = f"unimol-v2-{model_size}"
        model_name = 'unimolv2'
    ...
    print(f"Computing {cache_name} embeddings...")
    from src.embedding.unimol import UniMolEmbedder
    embedder = UniMolEmbedder(
        model_name=model_name,
        model_size=model_size if model_name == 'unimolv2' else '84m',
        device='cpu',
        use_gpu=False
    )
...
def main():
    ...
    parser.add_argument('--model-size', default='84m', help='For v2: 84m, 164m, 310m, 570m, 1.1B')
    args = parser.parse_args()

    label = f"Uni-Mol {'v1 (512d)' if args.version == 'v1' else f'v2-{args.model_size}'}"
    result_key = f"unimol-{'v1' if args.version == 'v1' else f'v2-{args.model_size}'}"

    print("=" * 60)
    print(f"{label} Phase 1 Evaluation")
    print("=" * 60)
    ...
    t0 = time.time()
    emb_dict, emb_dim = cache_unimol_embeddings(all_smiles, args.version, args.model_size)
    ...
    if "phase1" not in all_results:
        all_results["phase1"] = {}
    all_results["phase1"][result_key] = result
