#!/usr/bin/env python3
"""
Mol 1 REINVENT4 Expansion — Run all 3 generators (Mol2Mol, De Novo, LibInvent)
focused on Mol 1, then collect, score, merge with Phase 1 results, and generate
a unified HTML report.

Mol 1 (free base): C=CC(=O)N1Cc2cccc(C(=O)Nc3cn(C(C)C)cn3)c2C1
Pyrimidine-piperazine with acrylamide warhead.

Usage:
    conda run --no-capture-output -n quris python -u experiments/run_mol1_reinvent4.py

Phase 2a: REINVENT4 Mol2Mol — analogs of Mol 1
Phase 2b: REINVENT4 De Novo — constrained to similarity to Mol 1
Phase 2c: REINVENT4 LibInvent — R-group decoration on Mol 1 scaffold
...
MOL1_SEED_FILE = RESULTS_DIR / "mol1_seed.smi"
...
def create_mol2mol_config(output_path):
    """Create Mol2Mol config focused on Mol 1 as seed."""
    config = f'''# REINVENT4 Mol2Mol: Mol 1 Expansion
...
[[stage]]
chkpt_file = "mol1_mol2mol_s1.chkpt"
termination = "simple"
max_score = 0.7
min_steps = 10
max_steps = 50
...
[[stage]]
chkpt_file = "mol1_mol2mol_s2.chkpt"
termination = "simple"
max_score = 0.75
min_steps = 10
max_steps = 30
...
def create_denovo_config(output_path):
    """Create De Novo config with similarity constraint to Mol 1."""
    config = f'''# REINVENT4 De Novo: Mol 1-guided generation
run_type = "staged_learning"
device = "cpu"
tb_logdir = "tb_mol1_denovo"
json_out_config = "_mol1_denovo.json"

[parameters]
summary_csv_prefix = "mol1_denovo"
use_checkpoint = false
purge_memories = false
...
# Stage 1: Explore around Mol 1 chemistry
[[stage]]
chkpt_file = "mol1_denovo_s1.chkpt"
termination = "simple"
max_score = 0.65
min_steps = 50
max_steps = 200
...
# Similarity to Mol 1 — keep de novo anchored
[[stage.scoring.component]]
[stage.scoring.component.TanimotoDistance]
[[stage.scoring.component.TanimotoDistance.endpoint]]
name = "Similarity to Mol18"
...
[[stage]]
chkpt_file = "mol1_denovo_s2.chkpt"
termination = "simple"
max_score = 0.75
min_steps = 25
max_steps = 150
...
def create_libinvent_config(output_path, scaffold_file):
    """Create LibInvent config for Mol 1 scaffold decoration."""
    config = f'''# REINVENT4 LibInvent: Mol 1 scaffold decoration
run_type = "staged_learning"
device = "cpu"
tb_logdir = "tb_mol1_libinvent"
json_out_config = "_mol1_libinvent.json"

[parameters]
summary_csv_prefix = "mol1_libinvent"
use_checkpoint = false
purge_memories = false
...
# Stage 1: Decorate Mol 1 scaffold
[[stage]]
chkpt_file = "mol1_libinvent_s1.chkpt"
termination = "simple"
max_score = 0.65
min_steps = 50
max_steps = 200
...
def prepare_mol1_scaffold():
    """Prepare Mol 1 scaffold with attachment points for LibInvent.

    Mol 1: C=CC(=O)N1Cc2cccc(C(=O)Nc3cn(C(C)C)cn3)c2C1
    Core: isoindoline + aryl carboxamide-imidazole. Decoration sites at the
    isoindoline N (acrylamide warhead) and the imidazole N-substituent.
    """
    # Two decoration sites on the isoindoline + imidazole-carboxamide core:
    # [*:0] at the isoindoline N (replaces acrylamide warhead)
    # [*:1] at the imidazole N (replaces isopropyl)
    scaffold_variants = [
        # Both warhead and imidazole-N open
        "[*:0]N1Cc2cccc(C(=O)Nc3cn([*:1])cn3)c2C1",
        # Only warhead position open (keep imidazole N-isopropyl)
        "[*:0]N1Cc2cccc(C(=O)Nc3cn(C(C)C)cn3)c2C1",
        # Both isoindoline N and aryl-carboxamide open (more aggressive)
        "[*:0]N1Cc2cccc([*:1])c2C1",
    ]

    # Use the 2-attachment-point version for maximum diversity
    chosen = scaffold_variants[0]
    ...
    scaffold_file = REINVENT_RESULTS_DIR / "mol1_scaffold.smi"
...
def compute_properties(smiles_list):
    ...
    mol1_fp = AllChem.GetMorganFingerprintAsBitVect(
        Chem.MolFromSmiles(MOL1_SMILES), 2, nBits=2048)
...
<html><head>
<meta charset="utf-8">
<title>Mol 1 Expansion — Unified Report</title>
<style>
...
<h1>Mol 1 Expansion Pipeline — Unified Report</h1>
...
<div class="seed-card">
    <div style="display:flex; gap:30px; align-items:center;">
        <div>{mol_to_svg(MOL1_SMILES, (300, 250))}</div>
        <div>
            <h3 style="margin-top:0;">Mol 1 (Free Base)</h3>
            <p><code style="font-size:0.9em;">{MOL1_SMILES}</code></p>
            <p>Pyrimidine-piperazine core with acrylamide warhead</p>
            <p>MW: {Descriptors.MolWt(Chem.MolFromSmiles(MOL1_SMILES)):.1f} |
               LogP: {Descriptors.MolLogP(Chem.MolFromSmiles(MOL1_SMILES)):.2f} |
               QED: {QED.qed(Chem.MolFromSmiles(MOL1_SMILES)):.3f} |
               HBA: {Descriptors.NumHAcceptors(Chem.MolFromSmiles(MOL1_SMILES))} |
               HBD: {Descriptors.NumHDonors(Chem.MolFromSmiles(MOL1_SMILES))}</p>
        </div>
    </div>
</div>
...
<li>Candidates with higher Tc to Mol 1 and to ZAP70 training set have more trustworthy predictions</li>
...
def main():
    ...
    log("=" * 70)
    log("Mol 1 REINVENT4 Expansion Pipeline")
    log("=" * 70)
    ...
    mol2mol_config = mol2mol_dir / "mol1_mol2mol.toml"
    mol2mol_dir.mkdir(parents=True, exist_ok=True)

    # Check if already completed
    mol2mol_csvs = list(mol2mol_dir.glob("mol1_mol2mol*.csv"))
    if mol2mol_csvs:
        log("  Mol2Mol: SKIPPING (already completed)")
        mol2mol_ok = True
    else:
        create_mol2mol_config(mol2mol_config)
        mol2mol_ok = run_reinvent4(mol2mol_config, "mol1_mol2mol", mol2mol_dir)

    mol2mol_results = []
    if mol2mol_ok:
        mol2mol_results = collect_reinvent_results(mol2mol_dir, "mol1_mol2mol")
    ...
    denovo_config = denovo_dir / "mol1_denovo.toml"
    denovo_dir.mkdir(parents=True, exist_ok=True)

    denovo_csvs = list(denovo_dir.glob("mol1_denovo*.csv"))
    if denovo_csvs:
        log("  De Novo: SKIPPING (already completed)")
        denovo_ok = True
    else:
        create_denovo_config(denovo_config)
        denovo_ok = run_reinvent4(denovo_config, "mol1_denovo", denovo_dir)

    denovo_results = []
    if denovo_ok:
        denovo_results = collect_reinvent_results(denovo_dir, "mol1_denovo")
        for r in denovo_results:
            r["method"] = "De Novo"
    log(f"  De Novo: {len(denovo_results)} molecules")

    # ── Phase 2c: LibInvent ───────────────────────────────────────────────
    scaffold_file = prepare_mol1_scaffold()
    libinvent_dir = REINVENT_RESULTS_DIR / "libinvent"
    libinvent_config = libinvent_dir / "mol1_libinvent.toml"
    libinvent_dir.mkdir(parents=True, exist_ok=True)
    create_libinvent_config(libinvent_config, scaffold_file)
    libinvent_ok = run_reinvent4(libinvent_config, "mol1_libinvent", libinvent_dir)

    libinvent_results = []
    if libinvent_ok:
        libinvent_results = collect_reinvent_results(libinvent_dir, "mol1_libinvent")
    ...
    if SCORED_CSV.exists():
        ...
        # Phase 1 CSV uses film_pIC50/source/tanimoto_to_mol1 columns
        for _, row in p1_df.iterrows():
            ...
            phase1_candidates.append({
                ...
                "Tc_to_Mol18": row.get("tanimoto_to_mol1", row.get("Tc_to_Mol18", None)),
            })
    ...
    # Remove Mol 1 itself
    mol1_can = Chem.MolToSmiles(Chem.MolFromSmiles(MOL1_SMILES))
    all_candidates = [c for c in all_candidates if c.get("smiles") != mol1_can]
