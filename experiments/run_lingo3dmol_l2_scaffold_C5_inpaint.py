"""L2 SCAFFOLD INPAINT (C5 variant) — clamp recognition scaffold with the open
valence on the AROMATIC C5 of the isoindoline, not the aliphatic C1.

Motivation (medchem reviewer): the existing scaffold-anchor cohort uses
`C=CC(=O)N1Cc2ccccc2C1[*]` — [*] sits on the sp3 C1, so the arm decodes
out-of-plane (toward solvent / αC helix), NOT toward Met414 hinge. The
MM-GBSA winners attach the arm at the AROMATIC C5 instead, i.e.
`C=CC(=O)N1Cc2cccc(R)c2C1`. These are structural isomers — the chassis is
geometrically wrong in the C1 cohort.

This script copies the C1 scaffold-inpaint workflow but:
  * Same 13-heavy-atom ETKDG embed (the scaffold *molecule* without [*] is
    identical between C1 and C5 variants).
  * Different FSMILES token order: traversal goes through the aromatic ring
    BEFORE closing the 5-ring, and places `[*]_0` as a BRANCH off the
    aromatic carbon (atom 10) — the C5 position meta to the CH2 bridge.

Scaffold prefix breakdown (FSMILES vocab from util/fragmol_frag_zyh.py):

  SMILES:   C  =  C  C  (  =  O  )  N1 C  c2 c  c  c  c  (  [*]  )  c2 C1
  token:    C_0 =_0 C_0 C_0 (_0 =_0 O_0 )_0 N_5 1_0 C_5 c_5 2_0 c_6 c_6 c_6 c_6 (_0 [*]_0 )_0 c_5 2_0 C_5 1_0
  vocab id: 4   68  4   4   70  68  35  71  17  61  5   11  62  12  12  12  12  70  74    71  11  62  5   61

Wrapped: [start_0=1] + scaffold_tokens + [sep_0=3]  → 26 tokens total.
...
Atom indices in the 2D scaffold (no [*]):
   ...
   6  c   (benzene junction near CH2 — ring2 opens)
   7  c   (benzene)
   8  c   (benzene)
   9  c   (benzene)
   10 c   (benzene — THIS atom carries the [*] branch; meta to CH2 bridge)
   11 c   (benzene junction near sp3 CH — ring2 closes)
   12 C   (isoindoline CH, ring atom; closes ring1; NO substituent)

13-heavy-atom scaffold (without [*]) gets ETKDG-embedded, then translated so
atom 0 sits at cb_pos_target and atom0→atom1 aligns with the Bürgi-Dunitz
attack vector — same recipe as L2 v3.

Run target:
  conda run -n lingo3dmol python experiments/run_lingo3dmol_l2_scaffold_C5_inpaint.py \
      --pocket_pdb data/lingo3dmol_smoke/zap70_pocket_cys346.pdb \
      --output    data/lingo3dmol_L2_scaffold_C5/samples_T10.sdf \
...
# --- Scaffold token sequence (C5 attachment) ------------------------
# SMILES traversal: C=CC(=O)N1Cc2cccc([*])c2C1
# Convention: fused 5/6 junction atoms (idx 6 & 11) are tagged `c_5`.
SCAFFOLD_TOKEN_LABELS = [
    ...
    'c_5',           # atom 6  (benzene junction near CH2; fused to 5-ring)
    '2_0',
    'c_6',           # atom 7  (benzene)
    'c_6',           # atom 8  (benzene)
    'c_6',           # atom 9  (benzene)
    'c_6',           # atom 10 (benzene — C5 of isoindoline; carries [*] branch)
    '(_0',
    '[*]_0',         # open valence at aromatic C5
    ')_0',
    'c_5',           # atom 11 (benzene junction near sp3 CH; closes ring2)
    '2_0',
    'C_5',           # atom 12 (isoindoline CH; closes ring1)
    '1_0',
    'sep_0',
]
...
ATOM_TOKEN_INDEX_TO_ATOM = {
    ...
    15: 8,  # c_6
    16: 9,  # c_6
    17: 10, # c_6  (C5 — [*] hangs off this)
    21: 11, # c_5 (junction)
    23: 12, # C_5 (sp3 CH)
}

# Sanity: decode the scaffold token sequence at import time.
...
print(f"[L2 scaffold C5] scaffold decode sanity: {_smi_chk[0] if _smi_chk else None}")
...
print(f"[L2 scaffold C5] canonical:                {_canon}")
_expected = Chem.MolToSmiles(Chem.MolFromSmiles('C=CC(=O)N1Cc2cccc([*])c2C1'))
print(f"[L2 scaffold C5] expected canonical:       {_expected}")
assert _canon == _expected, f"scaffold mismatch: got {_canon!r} expected {_expected!r}"

# Verify the [*] sits on an AROMATIC carbon in the decoded mol.
_check_mol = Chem.MolFromSmiles(_smi_chk[0])
_star_atom = next((a for a in _check_mol.GetAtoms() if a.GetSymbol() == '*'), None)
assert _star_atom is not None, "no [*] atom in decoded prefix"
_star_neighbors = list(_star_atom.GetNeighbors())
assert len(_star_neighbors) == 1, f"expected 1 neighbor for [*], got {len(_star_neighbors)}"
_attach = _star_neighbors[0]
print(f"[L2 scaffold C5] [*] attaches to: {_attach.GetSymbol()} aromatic={_attach.GetIsAromatic()} in_ring={_attach.IsInRing()}")
assert _attach.GetIsAromatic(), (
    f"REGIOCHEMISTRY BUG: [*] sits on non-aromatic atom "
    f"({_attach.GetSymbol()} aromatic={_attach.GetIsAromatic()}). "
    f"This is the bug we are trying to fix — token list is wrong."
)
print("[L2 scaffold C5] [*] confirmed on aromatic C — PASS")
...
def _build_scaffold_xyz(anchor_json_path):
    ...
    try:
        ...
        print(f"[L2 scaffold C5] MMFF relax failed (continuing): {e}")
    ...
    print(f"[L2 scaffold C5] BD angle at atom0 = {measured:.2f} deg "
...
def run(args):
    ...
    print(f"[L2 scaffold C5] pocket={args.pocket_pdb}")
    print(f"[L2 scaffold C5] anchor={args.anchor_json}")
    print(f"[L2 scaffold C5] gennums={args.gennums} min_acceptable={args.min_acceptable}")
    print(f"[L2 scaffold C5] tempture={args.tempture}")
    print(f"[L2 scaffold C5] scaffold prefix ({len(SCAFFOLD_TOKENS)} tokens) = {SCAFFOLD_TOKENS}")
    print(f"[L2 scaffold C5] scaffold labels                                 = {SCAFFOLD_TOKEN_LABELS}")
    ...
    print(f"[L2 scaffold C5] pocket_center={pocket_center.tolist()}")
    print(f"[L2 scaffold C5] scaffold xyz (Å):")
    for i, p in enumerate(scaffold_xyz):
        print(f"    atom {i}: ({p[0]:.3f}, {p[1]:.3f}, {p[2]:.3f})")
    print(f"[L2 scaffold C5] scaffold grid (voxel):")
    for i, p in enumerate(scaffold_grid):
        print(f"    atom {i}: ({p[0]:.2f}, {p[1]:.2f}, {p[2]:.2f})")
    if (scaffold_grid.min() < 0) or (scaffold_grid.max() >= 240):
        print(f"[L2 scaffold C5] WARNING: grid coords outside [0,240): "
    ...
    print(f"[L2 scaffold C5] seeded {len(initial_partial[0][0])} tokens + "
    ...
    summary = {
        ...
        "attachment_position": "aromatic_C5",
    }
    ...
    try:
        while len(results) < args.gennums and (time.time() - start) < args.max_run_seconds:
            ...
            if (len(results) >= args.min_acceptable
                ...
                print(f"[L2 scaffold C5] min_acceptable={args.min_acceptable} met with "
    ...
    print(f"[L2 scaffold C5] generated {len(results)} mols in {elapsed:.1f}s status={status}")
    print(f"[L2 scaffold C5] wrote SDF: {args.output}")
